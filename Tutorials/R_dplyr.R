...
flights %>% filter(flight==1499) %>% select(origin,dest)
flights %>% mutate(dist_km = distance * 1.61, hours = air_time / 60, kph = dist_km/hours ) %>%
  select(flight, kph) %>% arrange(desc(kph))
