...
subset_id = outcome %>% top_n(500,T) %>% select(Gene)
subset_id = as.character(unname(t(subset_id[,1])))
```
...
for (j in 1:10){
  ...
  for (n_iter in n_iters){
    print(n_iter)
    myboost = ada(EFFECT~.,data=gene_sub_train_reduced[train_id,])
    test_pred = predict(myboost,gene_sub_test_reduced)
    accuracy[toString(n_iter)] = accuracy[toString(n_iter)] + mean(test_pred==gene_sub_test_reduced[['EFFECT']])
  }
}
```
...
```{r}
myboost = ada(EFFECT~.,data=gene_sub_train_reduced,iter=15)
```
